...
public class WXPayReport {
    ...
    private WXPayReport(final WXPayConfig config) {
        ...
        if (config.shouldAutoReport()) {
        	System.out.println("report worker num:"+config.getReportWorkerNum());
                    ...
                    public void run() {
                        while (true) {
                            ...
                            try {
                                ...
                                System.out.println("get first report msg:"+firstMsg);
                                String msg = null;
                                sb.append(firstMsg); //会阻塞至有消息
                                int remainNum = config.getReportBatchSize() - 1;
                                for (int j=0; j<remainNum; ++j) {
                                	System.out.println("try get remain report msg");
                                    // msg = reportMsgQueue.poll();  // 不阻塞了
                                    msg = reportMsgQueue.take();
                                    System.out.println("get remain report msg: "+msg);
                                    if (msg == null) {
                                        break;
                                    }
                                    else {
                            	...
                            	System.out.println("report fail. reason: " +ex.getMessage());
                            }
                        }
                    }
                });
